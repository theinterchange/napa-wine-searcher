...
export function UserAccommodationRating({
  accommodationId,
  accommodationName,
  size = "md",
}: {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/user/ratings/accommodation?accommodationId=${accommodationId}`
    )
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    const pending = consumePendingAction("rate-accommodation", accommodationId);
  ...
  const handleChange = (next: number) => {
    if (!session) {
      setPendingAction("rate-accommodation", accommodationId, {
        rating: next,
      });
  ...
  const persist = async (next: number) => {
    ...
    setRating(next);
    try {
      const res = await fetch("/api/user/ratings/accommodation", {
        method: next === 0 ? "DELETE" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          next === 0 ? { accommodationId } : { accommodationId, rating: next }
        ),
      });
      if (!res.ok) {
        setRating(prev);
  ...
  return (
    <>
      ...
      {showAuthModal && (
        <AuthGateModal
          message={`Sign in to save your rating for ${accommodationName}.`}
