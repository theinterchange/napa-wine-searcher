...
export default async function NapaValleyPage() {
  const [overview, varietals, heroImageUrl, accommodations, blogPosts] =
    await Promise.all([
      ...
      Promise.resolve(getRecentDistinctPosts(3, "napa valley")),
    ]);
        ...
        <section className="border-t border-[var(--rule-soft)]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <GuideHotelInline
              ...
              sourcePage="/napa-valley"
              sourceComponent="region_bookhotel_inline"
              lead="Planning a Napa weekend?"
            />
          </div>
        </section>
      )}
          ...
          <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-18 lg:px-8 lg:py-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {accommodations.map((a) => (
                <AccommodationCard key={a.slug} accommodation={a} showBookingCTA sourceComponent="NapaValleyRegionStay" />
              ))}
            </div>
          </div>
        </section>
