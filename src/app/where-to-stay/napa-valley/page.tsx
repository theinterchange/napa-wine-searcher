...
export const metadata: Metadata = {
  ...
  description:
    "Hand-picked hotels, inns, and resorts in Napa Valley. Curated for wine country visitors by location and proximity to the best wineries.",
  alternates: { canonical: `${BASE_URL}/where-to-stay/napa-valley` },
};

export default async function NapaValleyStayPage() {
  const accommodations = await getAllAccommodations("napa");
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      >
        ...
        <ChevronRight className="h-3 w-3" />
        <span className="text-[var(--brass-2)]">Napa Valley</span>
      </nav>

      <div className="mb-10 pb-6 border-b border-[var(--rule)]">
        <span className="kicker">Lodging · Napa Valley</span>
        <h1 className="editorial-h2 text-[34px] sm:text-[44px] lg:text-[52px] mt-3">
          Where to stay in <em>Napa.</em>
        </h1>
        <p className="mt-5 max-w-[56ch] font-[var(--font-serif-text)] text-[16px] sm:text-[18px] leading-[1.55] text-[var(--ink-2)]" style={{ textWrap: "pretty" }}>
          ...
          our picks for the best places to stay during your Napa Valley wine
          trip.
        </p>
      </div>
      ...
      {accommodations.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="font-[var(--font-serif-text)] text-[var(--ink-3)]">
            We&apos;re curating the best places to stay in Napa Valley.
            Check back soon.
          </p>
        </div>
      )}
