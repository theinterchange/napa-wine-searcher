/**
 * Catch-all route for the kid-friendly wineries category cluster.
 *
 *   /kid-friendly-wineries                       → cluster hub
 *   /kid-friendly-wineries/napa-valley           → Napa spoke
 *   /kid-friendly-wineries/sonoma-county         → Sonoma spoke
 *   /kid-friendly-wineries/{subregion-slug}      → subregion spoke
 *
 ...
 * subregion key is present in KID_META in category-content.ts. Anything
 * else returns notFound() — no thin pages.
 */
...
const AMENITY = "kid" as const;
const ROOT_PATH = "/kid-friendly-wineries";
...
export default async function KidFriendlyWineriesPage({ params }: PageProps) {
  const { slug } = await params;
  const scope = await resolveScope(slug);
  if (!scope) notFound();
