/**
 * Catch-all route for the dog-friendly wineries category cluster.
 *
 *   /dog-friendly-wineries                       → cluster hub
 *   /dog-friendly-wineries/napa-valley           → Napa spoke
 *   /dog-friendly-wineries/sonoma-county         → Sonoma spoke
 *   /dog-friendly-wineries/{subregion-slug}      → subregion spoke
 *
 ...
 * subregion key is present in DOG_META in category-content.ts. Anything
...
const ROOT_PATH = "/dog-friendly-wineries";
...
export default async function DogFriendlyWineriesPage({ params }: PageProps) {
  const { slug } = await params;
  const scope = await resolveScope(slug);
  if (!scope) notFound();
