...
(async () => {
  const rs = await client.execute(`
    SELECT slug, name,
      ...
      kid_friendly, kid_friendly_source, kid_friendly_note,
      adults_only
  ...
  writeFileSync("/tmp/prod-accommodations.json", JSON.stringify(rows, null, 2));
  console.log(`Exported ${rows.length} accommodations`);
  process.exit(0);
})();
